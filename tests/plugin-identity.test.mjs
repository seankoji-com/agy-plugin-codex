...
describe("plugin identity helpers", () => {
  ...
  it("detects both legacy local and versioned marketplace cache entries", () => {
    ...
    fs.mkdirSync(path.join(codexHome, "plugins", "cache", "local-plugins", "agy", "local"), {
      recursive: true,
    });
    fs.mkdirSync(path.join(codexHome, "plugins", "cache", "sendbird", "agy", "1.0.8"), {
      recursive: true,
    });
  ...
  it("reads managed plugin signals from an explicit codex home", () => {
    ...
    fs.mkdirSync(path.join(codexHome, "plugins", "cache", "sendbird", "agy", "1.0.9"), {
      recursive: true,
    });
